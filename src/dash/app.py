...
def get_data():
    data = pd.read_csv('../../data/class_pred.csv', index_col=0)
...
st.sidebar.info(
    "Ссылка на датасет: https://drive.google.com/file/d/1YXPZeTJ6XALPJnA95TiK8jb4odI_FH9t/view"
)
st.sidebar.info("Мой "
                "[github](https://github.com/melskiy)")
...
if selected2 == "Визуализация":
    ...
    if selected3 == "Диаграмма рассеяния":
        ...
        fig = create_scatterplotmatrix(datasmal[['Time','Length']], diag='histogram', height=800, width=800)
        st.plotly_chart(fig)
